...
class encoder_CLASSICDN:
	...
	def build_model(self, input_shape, nb_classes):
		...
		emb = keras.layers.Dense(10, activation='relu')(Dense2)
		
		Dense3 = keras.layers.Dense(32, activation='relu')(emb)
	...
	def fit(self, x_train, y_train, x_val, y_val,y_true,test_spe=False): 
		...
		hist = self.model.fit(x_train_noisy, x_train, batch_size=mini_batch_size, epochs=nb_epochs,
		...
		emb = self.emb.predict(x_val_noisy)
		...
		if test_spe:
			...
			for atk in l_atk:
				...
				#decod = self.decoder.predict(x_val)
				emb = self.emb.predict(x_val_noisy)
				tresh_atk = np.percentile(y_pred,20)
                #y_pred_emb = np.array([np.linalg.norm(a-b) for a,b in zip(encod, decod)])
				#print("embeding vector shape is {}".format(encod.shape))
				#print(y_pred.shape, y_true.shape)

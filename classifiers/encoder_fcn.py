...
class encoder_FCN:
	...
	def build_model(self, input_shape, nb_classes):
		...
		conv1 = keras.layers.Conv1D(filters=16, kernel_size=16,strides=5 ,padding='same',activation='relu')(input_layer)
		#conv1 = keras.layers.MaxPool1D(2)(conv1)
		
		conv2 = keras.layers.Conv1D(filters=8, kernel_size=8, padding='same',activation='relu')(conv1)
		#conv2 = keras.layers.MaxPool1D(2)(conv2)
		flat = keras.layers.Flatten()(conv2)
		emb = keras.layers.Dense(10, activation='relu')(flat)
		
		deconv3 = keras.layers.Conv1D(filters=8, kernel_size=8,padding='same', activation='relu')(conv2)
		#deconv3_mx = keras.layers.UpSampling1D(5)(deconv3)
		

		deconv4 = keras.layers.Conv1D(filters=16, kernel_size=16, padding='same',activation='relu')(deconv3)
		deconv4 = keras.layers.UpSampling1D(5)(deconv3)
		#deconv4 = keras.layers.UpSampling1D(2)(deconv4)
		#flat = keras.layers.Flatten()(deconv4)
		output = keras.layers.Dense(input_shape[1],activation='sigmoid')(deconv4)
